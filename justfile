...
# remove generated test migrations (the frozen legacy migration is kept)
[script]
_rm-test-migrations:
    from pathlib import Path
    for migration in Path("tests").glob("*/migrations/00*.py"):
        if migration != Path("tests/legacy_migrations/migrations/0001_initial.py"):
            migration.unlink()

# regenerate test migrations using the lowest version of Django
remake-test-migrations: _rm-test-migrations
    @just make-test-migrations

# regenerate the frozen tests/legacy_migrations initial migration with an old release of django-enum
[script]
make-legacy-migrations VERSION="2.5.0" PYTHON="3.14":
    import os
    import subprocess
    from pathlib import Path
    migrations = Path("tests/legacy_migrations/migrations")
    for migration in migrations.glob("00*.py"):
        migration.unlink()
    subprocess.run(
        [
            "uv", "run", "--isolated", "--no-project", "--python", "{{ PYTHON }}",
            "--with", "django-enum=={{ VERSION }}", "--with", "django>=5.2,<6",
            "django-admin", "makemigrations", "tests_legacy_migrations", "--name", "initial",
        ],
        check=True,
        env={
            **os.environ,
            "PYTHONPATH": os.getcwd(),
            "DJANGO_SETTINGS_MODULE": "tests.legacy_migrations.v2.settings",
        },
    )
    initial = migrations / "0001_initial.py"
    initial.write_text(
        "# Generated by django-enum {{ VERSION }} (Python {{ PYTHON }}, Django 5.2). "
