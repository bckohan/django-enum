...
def plot_size(size_benchmarks):
    ...
    for vendor, metrics in size_benchmarks.items():
        ...
        if (
            metrics['bools'].get('column', []) and
            metrics['flags'].get('column', [])
        ):
            num_flags = []
            bytes_saved = []

            for idx, (bools, flags) in enumerate(zip(
                    metrics['bools']['column'],
                    metrics['flags']['column']
            )):
                num_flags.append(idx + 1)
                bytes_saved.append((bools - flags) / count)
...
def plot_no_index_comparison(queries, rdbms='postgres', num_flags=16):
    ...
    if not (bool_no_index and flags_no_index):
        raise ValueError(
            f'No "No Index data" found for {rdbms}: {num_flags} flags'
        )
    ...
    for label, counts, style in [
        ...
        for qry, plt_data in plts.items():
            plt.plot(cnts, plt_data[0], f'{plt_data[1]}{style}', label=f'[{label}] {qry}')
...
def plot_exact_index_comparison(queries, rdbms='postgres', num_flags=16):
    ...
    if not (bool_no_index and flags_no_index):
        raise ValueError(
            f'No "Exact Query" data found for {rdbms}: {num_flags} flags'
        )
    ...
    for label, counts, style in [
        ...
        for count in cnts:
            ...
            for key, qry, color in lines:
                if key in metrics:
                    plts[qry][0].append(
                        metrics[key] / (1024**3)
                        if qry == 'table_size' else metrics[key]
                    )
        ...
        for qry, plt_data in plts.items():
            ...
            if qry == 'table_size':
                axis = size_plt
            handles.append(
                axis.plot(cnts, plt_data[0], f'{plt_data[1]}{style}', label=f'[{label}] {qry}')[0]
            )
...
def plot_table_sizes(queries, rdbms='postgres', num_flags=16):
    ...
    if not lines:
        raise ValueError(
            f'No table size data found for {rdbms}: {num_flags} flags'
        )
